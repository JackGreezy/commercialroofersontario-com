...
const contactConfig = {"name": "Commercial Roofers of Ontario", "domain": "commercialroofersontario.com", "address": "3281 E Guasti Rd, Suite 700, Ontario, CA 91761", "phone": "", "phoneTel": "", "email": "repairs@commercialroofersontario.com", "city": "Ontario", "state": "California"};
...
module.exports = async function handler(req, res) {
  ...
  try { await sendLeadEmails(lead, req); } catch (error) { console.error("Contact email send failed", error); const message = error && error.message === "SENDGRID_API_KEY is missing." ? "Email service is not configured." : "We could not submit your request right now. Please email repairs@commercialroofersontario.com."; return sendJson(res, 500, { ok: false, success: false, message, error: "email-send-failed" }, headers); }
